...
def test_multiple_types(db):
    db.put_int(b"int", 1)
    db.put_str(b"str", "hello")
    db.put_float(b"float", 1.0)
    assert db.get_int(b"int") == 1
    assert db.get_str(b"str") == "hello"
    assert db.get_float(b"float") == 1.0
    with pytest.raises(TypeError):
        db.put_int(b"int", "not int")
    with pytest.raises(TypeError):
        db.put_str(b"str", 1)
    with pytest.raises(TypeError):
        db.put_float(b"float", "not float")
