...
impl<T: Send + std::fmt::Debug> MessageDispatcher<T> {
    ...
    async fn run_lifecycle(
        ...
        closed_result.write_close_result(run_result.clone());
        ...
        while !self.tasks.is_empty() {
            if let Some(((group_name, _), join_result)) = self.tasks.join_next().await {
                ...
                if let Err(ref err) = task_result {
                    ...
                    self.closed_result.write_close_result(task_result.clone());
...
impl MessageHandlerExecutor {
    ...
    /// ## Arguments
    ///
    /// * `closed_result`: A [DbStatusManager] that stores the database close result
    ...
    pub(crate) fn new(closed_result: DbStatusManager, clock: Arc<dyn SystemClock>) -> Self {
        Self {
...
mod test {
    ...
    async fn test_executor_propagates_handler_error_drains_messages() {
        ...
        let closed_result = DbStatusManager::new(0);
        ...
        assert!(matches!(
            closed_result.closed_result_reader().read(),
