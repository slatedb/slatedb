...
mod tests {
    ...
    async fn test_compacted_gc_evicts_deleted_sst_from_object_store_cache() {
        ...
        let format = SsTableFormat::default();
        // The GC store deletes through the cache; a main store caches the SST on
        // write.
        ...
        // Write the SST through the Main store so cache_on_flush admits it to the
        // object store cache.
        let id_to_delete = SsTableId::Compacted(ulid::Ulid::from_parts(1_000, 0));
        ...
        assert!(
            entry.cached_parts().await.unwrap().is_empty(),
            "sst should be evicted from the object store cache after delete"
        );
    }
}
