...
impl Manifest {
    ...
    pub(crate) fn cloned_from_union(
        ...
        // Coalesce borrows of the same physical ancestor. The dedup key is
        // (path, sst_ids), NOT (path, source_checkpoint_id): the carried-forward
        // source_checkpoint_id is the parent's final_checkpoint_id, which is
        // regenerated as a fresh random UUID on every cloned_from_union. Keying
        // on it would leave borrows of the same ancestor with identical sst_ids
        // un-coalesced whenever they came from independently-created clones (the
        // real rescale topology), making external_dbs grow 1:1 with union fan-in
        // and accumulate without bound across clone/union generations. Borrows
        // with identical sst_ids reference the exact same physical SSTs, so
        // collapsing them is safe: any member's source checkpoint already pins
        // those SSTs in the ancestor. We keep the smallest source_checkpoint_id
        // of the group for deterministic manifest bytes / UUID assignment.
            ...
            .fold(
                ...
                |mut map: BTreeMap<(String, BTreeSet<SsTableId>), Uuid>, db| {
                    ...
                    map.entry((db.path, sst_ids))
                        .and_modify(|cp| *cp = (*cp).min(db.source_checkpoint_id))
...
mod tests {
    ...
    fn test_union_collapses_identical_borrows_of_same_ancestor() {
        ...
        let merged_ids: BTreeSet<SsTableId> =
            shared_entries[0].sst_ids.iter().copied().collect();
    ...
    fn test_union_keeps_distinct_sst_sets_of_same_ancestor_separate() {
        ...
        let result =
            Manifest::cloned_from_union(sources, Arc::new(DbRand::default())).unwrap();
    ...
    fn repro_union_collapses_shared_ancestor_with_distinct_finals() {
        ...
        // (differing) source_checkpoint_ids.
        let final_cp_1 = Uuid::from_u128(1);
        let final_cp_2 = Uuid::from_u128(2);
    ...
    fn test_projected_drops_sorted_run_sst_when_range_falls_in_key_gap() {
        // Regression (NGN-1751): a projection range that lands entirely in the
        // gap between a sorted-run SST's last physical key and the next SST's
        // start key must drop the SST, not panic. `compacted_intersection`
        // bounds the SST by the next SST's start key (logical), so it reports
        // overlap for `(c, m)`, but the SST has no physical keys there.
