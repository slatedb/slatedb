    ...
    async fn execute_compaction_job(
        ...
        while let Some(kv) = all_iter.next_entry().await? {
            ...
            if duration_since_last_report > TimeDelta::seconds(1) {
                ...
                if let Err(e) = self.worker_tx.send(CompactorMessage::CompactionJobProgress {
                    id: args.id,
                    bytes_processed: all_iter.total_bytes_processed(),
                }) {
                    debug!("failed to send compaction progress (likely DB shutdown) [error={:?}]", e);
                }
    ...
    fn start_compaction_job(self: &Arc<Self>, args: StartCompactionJobArgs) {
        ...
        let task = spawn_bg_task(
            ...
            move |result| {
                ...
                {
                    debug!("failed to send compaction finished msg (likely DB shutdown) [error={:?}]", e);
                }
...
mod tests {
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn test_compaction_job_handles_closed_channel_gracefully() {
        // This test validates that the compactor doesn't panic when the channel
        // is closed (e.g., during DB shutdown). The fix replaces .expect() with
        // graceful error handling.
        let options = Arc::new(CompactorOptions::default());
        let (tx, rx) = tokio::sync::mpsc::unbounded_channel();
        let os = Arc::new(InMemory::new());
        let path = "testdb_closed_channel".to_string();
        let clock = Arc::new(DefaultSystemClock::new());
        let db = Db::builder(path.clone(), os.clone())
            .with_system_clock(clock.clone())
            .build()
        ...
        // Create a separate runtime for the executor's stop() to avoid nested block_on
        let manifest_store_clone = manifest_store.clone();
        let executor_handle = std::thread::spawn(move || {
            ...
            let executor = TokioCompactionExecutor::new(
                ...
                Arc::new(CompactionStats::new(Arc::new(StatRegistry::new()))),
                clock,
                manifest_store_clone,
                None,
            );
            (rt, executor)
        }).join().unwrap();
        
        let (rt, executor) = executor_handle;
        ...
        // Close the receiver to simulate DB shutdown
        drop(rx);

        // Start a compaction - this should not panic even though the channel is closed
        let compaction = StartCompactionJobArgs {
            id: Ulid::new(),
            compaction_id: Ulid::new(),
            destination: 0,
        ...
        // Give the compaction time to run and attempt to send messages
        tokio::time::sleep(Duration::from_millis(200)).await;

        // Stop the executor in a separate thread - should not panic
