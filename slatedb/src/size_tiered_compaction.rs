...
mod tests {
    ...
    fn test_borrowed_sst_views_join_compactable_run() {
        // Two borrowed halves of one ~1MB physical SST should each report
        // ~500KB and group with a genuine ~500KB peer, not each claim the
        // full ~1MB and get rejected as size-dissimilar.
        let parent = create_sst_view_with_bounds(b"a", b"z", 1_000_000);
        ...
        // A genuinely ~500KB SST, unrelated to the borrowed parent.
        let peer = create_sst_view_with_bounds(b"aa", b"az", 500_000);

        let sources = vec![
            CompactionSource {
                source: SourceId::SstView(borrowed_first_half.id),
                size: borrowed_first_half.estimate_size(),
            },
            CompactionSource {
                source: SourceId::SstView(borrowed_second_half.id),
                size: borrowed_second_half.estimate_size(),
            },
            CompactionSource {
                source: SourceId::SstView(peer.id),
                size: peer.estimate_size(),
            },
        ];
        ...
        assert_eq!(
            run.len(),
            3,
            "borrowed halves should now report ~500KB each and group with a \
             genuine ~500KB peer within the 4.0x size threshold, got sizes {:?}",
    ...
    fn test_propose_compacts_sorted_run_mixing_borrowed_and_plain_views() {
        // Goes through the real `propose()` -> `tree.srs` path (rather than
        // calling `build_compactable_run` directly) with a `SortedRun` that
        // mixes a plain and a borrowed/projected view, to check the
        // aggregate `SortedRun::estimate_size()` produces is itself sane.
        let scheduler = SizeTieredCompactionScheduler::default();

        let plain_view = create_sst_view_with_bounds(b"a", b"z", 50_000);

        // A 1MB physical SST projected down to a genuine 5% slice
        // (counter 0..10 of 0..=200) -> ~50KB visible.
        ...
        let mixed_sr_size = mixed_sr.estimate_size();
        assert!(
        ...
        // Three genuinely ~100KB peer sorted runs, similar enough (within
        // the 4.0x default `include_size_threshold`) to the mixed run above
        // to satisfy the default `min_compaction_sources` of 4.
        let state = &create_compactor_state(create_db_state(
            VecDeque::new(),
            vec![
                mixed_sr,
