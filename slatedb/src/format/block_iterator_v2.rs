...
use crate::format::block_iterator::BlockLike;
...
struct AscendingState<B: BlockLike> {
    block: B,
    /// Which restart region we're currently in
    current_restart_idx: usize,
    /// Current byte offset in the data section
    offset_in_block: usize,
    /// Number of entries read since the current restart point
    entries_since_restart: usize,
    /// The reconstructed key at the current position
    current_key: Bytes,
    /// Whether we've reached the end
    exhausted: bool,
}

/// Internal enum to hold either ascending or descending iteration state.
...
///
/// Supports both ascending and descending iteration orders. For descending
/// iteration, delegates to `DescendingBlockIteratorV2` internally.
...
/// Helper methods for ascending iteration state.
impl<B: BlockLike> AscendingState<B> {
    /// Position the iterator at a specific restart point.
    ...
    /// Decode the entry at the current offset and return it along with the new offset.
    ...
    /// Decode only the key at the current offset for seek comparisons.
    ...
    /// Find the restart point that contains or precedes the target key.
    /// Returns the earliest restart index with key >= target, or the previous restart if the
    /// target falls between restart keys.
    fn find_restart_for_key(&self, target: &[u8]) -> usize {
        ...
        // low is now the first restart point with key >= target.
        // Step back one restart to ensure we include any entries for target that
        // started in the previous restart region.
        if low == 0 {
            0
        } else {
            low - 1
    ...
    /// Find the last restart point with key <= target (used for descending seeks).
    fn find_restart_at_or_before_key(&self, target: &[u8]) -> usize {
        let restarts = self.block.offsets();
        if restarts.is_empty() {
            return 0;
        }
    ...
    /// Get the end offset for a restart region.
    fn restart_region_end(&self, restart_idx: usize) -> usize {
        let restarts = self.block.offsets();
        if restart_idx + 1 < restarts.len() {
            restarts[restart_idx + 1] as usize
        } else {
            self.block.data().len()
        }
    }

    /// Advance past the current entry by decoding and discarding it.
    /// Updates offset_in_block and entries_since_restart.
    ...
    async fn next_entry(&mut self) -> Result<Option<RowEntry>, SlateDBError> {
        match &mut self.inner {
            BlockIteratorInner::Ascending(state) => {
                ...
                if state.current_restart_idx + 1 < state.block.offsets().len() {
                    let next_restart_offset =
                        state.block.offsets()[state.current_restart_idx + 1] as usize;
                    if state.offset_in_block >= next_restart_offset {
    ...
    async fn seek(&mut self, next_key: &[u8]) -> Result<(), SlateDBError> {
        match &mut self.inner {
            BlockIteratorInner::Ascending(state) => {
                ...
                // Binary search to find the appropriate restart point
                let start_restart_idx = state.find_restart_for_key(next_key);

                // Outer loop: iterate through restart regions starting from binary search result.
                // In practice, this should only execute once since binary search finds the correct
                // restart point. However, we loop to confirm by checking the first key of the next
                // restart region, handling any edge cases where the target falls between regions.
                for restart_idx in start_restart_idx..state.block.offsets().len() {
                    // Position at the restart point
                    ...
                    // Check if the first key in this region is already >= target
                    if state.current_key.as_ref() >= next_key {
                        return Ok(());
                    }

                    // Inner loop: scan entries within this restart region
                        ...
                        && state.offset_in_block < state.block.data().len()
                    {
                        ...
                        if current_key.as_ref() >= next_key {
                            // Found a key >= target, position here
...
///
/// This iterator uses an `AscendingState` internally to decode entries within
/// each restart region, then caches them and iterates in reverse order.
pub(crate) struct DescendingBlockIteratorV2<B: BlockLike> {
    /// Ascending state used for decoding entries
    ascending: AscendingState<B>,
    /// Current restart region index (starts from the last)
    current_restart_idx: isize,
    /// Cached entries for the current restart region (in ascending order)
    cached_entries: Vec<RowEntry>,
    /// Index into cached_entries (iterates backwards)
    cache_idx: isize,
    /// Whether we've reached the end
    exhausted: bool,
}
...
impl<B: BlockLike> DescendingBlockIteratorV2<B> {
    pub(crate) fn new(block: B) -> Self {
        ...
        let mut iter = DescendingBlockIteratorV2 {
        ...
        if !iter.exhausted {
            iter.load_restart_region(iter.current_restart_idx as usize);
        }
    ...
    fn load_restart_region(&mut self, restart_idx: usize) {
        self.cached_entries.clear();

        let restarts = self.ascending.block.offsets();
        if restart_idx >= restarts.len() {
            self.cache_idx = -1;
            return;
        }

        // Position the ascending state at the restart point
        self.ascending.seek_to_restart(restart_idx);

        // Calculate the end of this restart region
        let region_end = self.ascending.restart_region_end(restart_idx);

        // Decode all entries in this region using the ascending state
        while self.ascending.offset_in_block < region_end && !self.ascending.is_empty() {
            if let Ok((entry, new_offset)) = self.ascending.decode_entry_at_current_offset() {
        ...
        // Set cache_idx to last element (for reverse iteration)
        self.cache_idx = self.cached_entries.len() as isize - 1;
    }

    /// Move to the previous restart region.
    fn move_to_previous_region(&mut self) {
    ...
    /// Find the restart point that contains or precedes the target key.
    fn find_restart_for_key(&self, target: &[u8]) -> usize {
        self.ascending.find_restart_at_or_before_key(target)
    }
}
...
impl<B: BlockLike> KeyValueIterator for DescendingBlockIteratorV2<B> {
    ...
    async fn next_entry(&mut self) -> Result<Option<RowEntry>, SlateDBError> {
        ...
        // If we have entries in the cache, return from the end
        if self.cache_idx >= 0 {
        ...
        // Cache exhausted, move to previous restart region
    ...
    async fn seek(&mut self, next_key: &[u8]) -> Result<(), SlateDBError> {
        ...
        // For descending seek, we want to find the largest key <= next_key
        // Start from the restart point that could contain this key
        let start_restart_idx = self.find_restart_for_key(next_key);

        // Search backwards through restart regions
        for restart_idx in (0..=start_restart_idx).rev() {
            ...
            for (i, entry) in self.cached_entries.iter().enumerate() {
                if entry.key.as_ref() <= next_key {
                    found_idx = i as isize;
                } else {
                    break; // Keys are sorted ascending, so we can stop
                }
            }

            if found_idx >= 0 {
                self.cache_idx = found_idx;
                self.exhausted = false;
                return Ok(());
            }
        }

        // No key <= next_key found
        self.exhausted = true;
        Ok(())
    }
}
...
mod tests {
    ...
    fn build_test_block_with_seqs(
        entries: &[(&[u8], &[u8], u64)],
        restart_interval: usize,
    ) -> Block {
        let mut builder = BlockBuilderV2::new_with_restart_interval(4096, restart_interval);
        for (key, value, seq) in entries.iter() {
            let _ = builder.add(make_entry(key, value, *seq));
        }
        builder.build().expect("build failed")
    }
    ...
    #[tokio::test]
    async fn should_seek_to_first_duplicate_across_restart_boundary() {
        // given: duplicate keys spanning restart boundaries
        let block = build_test_block_with_seqs(
            &[
                (b"dup", b"v4", 4), // restart 0
                (b"dup", b"v3", 3),
                (b"dup", b"v2", 2), // restart 1
                (b"dup", b"v1", 1),
                (b"next", b"v0", 0), // restart 2
            ],
            2,
        );
        ...
        // when: seeking to a key that has duplicates across restarts
        iter.seek(b"dup").await.unwrap();

        // then: returns the first duplicate entry (highest seq) without skipping
        let kv = iter.next_entry().await.unwrap().unwrap();
    ...
    #[tokio::test]
    async fn should_seek_descending_with_duplicate_keys_across_restart_boundary() {
        // given: duplicate keys spanning restart boundaries
        let block = build_test_block_with_seqs(
            &[
                (b"dup", b"v4", 4), // restart 0
        ...
        // when: seeking to the duplicate key in descending mode
        iter.seek(b"dup").await.unwrap();

        // then: returns the last duplicate entry (lowest seq) in descending order
        let kv = iter.next_entry().await.unwrap().unwrap();
        assert_eq!(kv.key.as_ref(), b"dup");
        assert_eq!(kv.seq, 1);
    }
