...
use crate::format::row_codec::RowFlags;
...
mod tests {
    ...
    #[rstest]
    #[case("value entry", 3, b"suffix".to_vec(), 100, Some(b"value".to_vec()), None, None)]
    #[case("entry with timestamps", 0, b"key".to_vec(), 1, Some(b"val".to_vec()), Some(1000), Some(2000))]
    #[case("tombstone", 5, b"tomb".to_vec(), 42, None, Some(500), None)]
    ...
    fn should_handle_tombstones() {
        ...
        // then: tombstone is preserved
        assert!(matches!(decoded.value, ValueDeletable::Tombstone));
        assert_eq!(decoded.create_ts, Some(12345));
    }
    ...
    #[test]
    fn should_decode_key_only_basic() {
        // given: an encoded entry with shared bytes
        let entry = SstRowEntryV2::new(
            5,
            Bytes::from("suffix"),
            1,
        ...
        let mut slice = buf.as_slice();
        let (shared_bytes, key_suffix) = codec.decode_key_only(&mut slice);

        // then: key info is correct
        assert_eq!(shared_bytes, 5);
        assert_eq!(key_suffix.as_ref(), b"suffix");
    }

    #[test]
    fn should_decode_key_only_at_restart_point() {
        // given: an entry at a restart point (shared_bytes = 0)
        let entry = SstRowEntryV2::new(
            0,
            Bytes::from("full_key_at_restart"),
            100,
            ValueDeletable::Value(Bytes::from("value")),
            None,
            None,
        ...
        // then: full key is returned with 0 shared bytes
        assert_eq!(shared_bytes, 0);
        assert_eq!(key_suffix.as_ref(), b"full_key_at_restart");
    }

    #[test]
    fn should_decode_key_only_with_empty_suffix() {
        // given: an entry where the entire key is shared (empty suffix)
        let entry = SstRowEntryV2::new(
            10,
            Bytes::new(), // empty suffix
            1,
            ValueDeletable::Value(Bytes::from("value")),
            None,
            None,
        ...
        // then: shared bytes is correct and suffix is empty
        assert_eq!(shared_bytes, 10);
        assert!(key_suffix.is_empty());
    }

    #[test]
    fn should_decode_key_only_with_large_shared_bytes() {
        // given: an entry with large shared_bytes requiring multi-byte varint
        let shared = 16384; // requires 3-byte varint
        let entry = SstRowEntryV2::new(
            shared,
            Bytes::from("x"),
            1,
            ValueDeletable::Value(Bytes::from("v")),
            None,
            None,
        );
        ...
        // then: large shared_bytes is decoded correctly
        assert_eq!(shared_bytes, shared);
        assert_eq!(key_suffix.as_ref(), b"x");
    }

    #[test]
    fn should_decode_key_only_with_tombstone() {
        // given: a tombstone entry (value_len = 0)
        let entry = SstRowEntryV2::new(
            3,
            Bytes::from("deleted"),
            999,
        ...
        // then: key info is correct regardless of value type
        assert_eq!(shared_bytes, 3);
        assert_eq!(key_suffix.as_ref(), b"deleted");
    }

    #[test]
    fn should_decode_key_only_advances_buffer_correctly() {
    ...
    #[test]
    fn should_decode_key_only_with_large_key_suffix() {
        // given: an entry with a large key suffix requiring multi-byte varint for length
        let large_suffix = vec![b'k'; 1000];
        let entry = SstRowEntryV2::new(
            0,
            Bytes::from(large_suffix.clone()),
            1,
            ValueDeletable::Value(Bytes::from("v")),
            None,
            None,
        ...
        // then: large key suffix is decoded correctly
        assert_eq!(shared_bytes, 0);
        assert_eq!(key_suffix.len(), 1000);
        assert_eq!(key_suffix.as_ref(), large_suffix.as_slice());
    }
    ...
    #[test]
    fn should_handle_merge_operand() {
        // given: a merge operand entry
        let entry = SstRowEntryV2::new(
            0,
            Bytes::from("merge_key"),
            42,
            ValueDeletable::Merge(Bytes::from("merge_value")),
            None,
            Some(1000),
        );
        ...
        // then: merge operand is preserved
        assert!(matches!(decoded.value, ValueDeletable::Merge(_)));
        if let ValueDeletable::Merge(v) = decoded.value {
            assert_eq!(v.as_ref(), b"merge_value");
        }
        assert_eq!(decoded.expire_ts, Some(1000));
    }
}
