...
impl GarbageCollector {
    ...
    pub async fn run_async_task(&self) -> Result<(), SlateDBError> {
        ...
        let compacted_ticker = self.system_clock.ticker(compacted_gc_task.interval());
        let wal_ticker = self.system_clock.ticker(wal_gc_task.interval());
        let manifest_ticker = self.system_clock.ticker(manifest_gc_task.interval());
        let log_ticker = self.system_clock.ticker(Duration::from_secs(60));
