...
use crate::compactor_state::{Compactions, CompactorState};
...
impl ManifestAndCompactionsReader {
    pub(crate) fn new(manifest_store: &Arc<ManifestStore>, compactions_store: &Arc<CompactionsStore>) -> Self {
        Self {
    ...
    pub(crate) async fn read_active_manifests_and_compactions(&self) -> Result<ActiveManifestAndCompactions, SlateDBError> {
...
impl CompactorStateWriter {
    ...
    pub(crate) async fn load_manifest(&mut self) -> Result<(), SlateDBError> {
        ...
        self.state.merge_remote_manifest(self.manifest.prepare_dirty()?);
        Ok(())
    }
