...
async fn flush_from_a_reader_flushes_the_shared_scope_including_the_live_writers_entries() {
    ...
    // Touch-tracking is per instance, not per scope (see
    // `DbCache::touched_sst_ids`), so the reader needs at least one read to
    // get this SST onto its own tracking before it can flush it.
