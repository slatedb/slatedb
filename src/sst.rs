...
mod tests {
    ...
    async fn test_sstable() {
        ...
        assert_eq!(b"key1", sst_info.first_key().unwrap().bytes(),"first key in sst info should be correct");
        assert_eq!(b"key1", sst_info.block_meta().get(0).first_key().bytes(), "first key in block meta should be correct");
