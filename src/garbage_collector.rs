...
impl GcTask for WalGcTask {
    ...
    async fn collect(&self, utc_now: DateTime<Utc>) -> Result<(), SlateDBError> {
        ...
        let last_compacted_wal_sst_id = latest_manifest.core.last_l0_recent_flushed_wal_sst_id;
...
mod tests {
    ...
    async fn test_collect_garbage_wal_ssts() {
        ...
        state.last_l0_recent_flushed_wal_sst_id = id2.unwrap_wal_id();
        StoredManifest::create_new_db(manifest_store.clone(), state.clone())
            .await
            .unwrap();
        ...
        assert_eq!(
            current_manifest.core.last_l0_recent_flushed_wal_sst_id,
            id2.unwrap_wal_id()
        );
    ...
    async fn test_do_not_remove_wals_referenced_by_active_checkpoints() {
        ...
        let mut state = CoreDbState::new();
        state.last_l0_recent_flushed_wal_sst_id = 1;
        ...
        let mut dirty = stored_manifest.prepare_dirty();
        dirty.core.last_l0_recent_flushed_wal_sst_id = 3;
    ...
    async fn assert_no_dangling_references(
        ...
        for manifest in manifests.values() {
            let wal_sst_start_inclusive = manifest.core.last_l0_recent_flushed_wal_sst_id + 1;
