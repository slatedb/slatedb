...
* Some object stores support compare-and-swap (CAS) operations or can be configured for conditional writes.
...
* **Operational simplicity**: SlateDB is easy to operate. It runs as an in-process storage engine with no control plane and no replication protocol. Local disk-backed caching is optional.
