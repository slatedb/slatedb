...
advancement, a metadata writer paused longer than `min_age` can recreate a deleted metadata ID and
incorrectly report its stale update as successful. Set the manifest and compactions `min_age` values
longer than the maximum lifetime of a stale process before using this mode.
