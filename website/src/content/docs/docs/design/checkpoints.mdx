...
Creating a checkpoint does not copy WAL or SST files. It only adds metadata to the manifest, so it is cheap. The pinned manifest still matters, though. It defines the L0 SSTs, sorted runs, and other database state that readers see.
