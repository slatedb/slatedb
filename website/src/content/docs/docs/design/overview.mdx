...
LSM trees are made up of a _write-ahead log (WAL)_, a mutable in-memory map called a _MemTable_, and _sorted strings tables (SSTables)_ on disk. We will explore each of these components in detail.
...
We've now built a basic LSM tree. We've glossed over many details (deletion tombstones, consistency, WAL replay, etc). Still, this basic implementation will work. If you're interested in experimenting more with LSMs, we highly recommend checking out [Mini-LSM](https://skyzh.github.io/mini-lsm/).
