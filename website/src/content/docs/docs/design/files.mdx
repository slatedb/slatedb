...
A SlateDB object store bucket contains five main directories:
...
The `gc` directory contains boundary files that act as durable high-watermarks to fence stale writes after garbage collection runs. Boundary files are named `<prefix>.boundary` and store ASCII-encoded monotonic IDs:

- `manifest.boundary` - Tracks the GC boundary for manifest files
- `compactions.boundary` - Tracks the GC boundary for compactions files

Each boundary file contains a single integer value representing the highest ID that has been durably fenced. When the garbage collector runs, it updates the boundary based on the `min_age` setting. Any attempt to write a sequenced file (manifest or compactions) with an ID at or below the boundary will be rejected, preventing data loss from stale writes.

Boundary files use conditional PUT operations with ETags to safely handle concurrent updates. After each successful write to manifest or compactions storage, the corresponding boundary is checked to ensure the new ID is greater than the current boundary value.
