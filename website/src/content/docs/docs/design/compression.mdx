...
Set [`Settings::compression_codec`](https://docs.rs/slatedb/latest/slatedb/config/struct.Settings.html#structfield.compression_codec) before opening the database:
...
The setting affects new WAL SSTs, new L0 SSTs, and new compacted SSTs. It does not rewrite old files immediately. Compaction eventually rewrites older compacted data with the current writer and compactor configuration.

Every process that reads a database must support every codec present in that database. That includes writers, readers, compactors, workers, `WalReader`, and direct SST inspection tools.
...
If you also use [Block Transforms](/docs/design/block-transformer/), compression runs before the transform on write and after the transform on read. That means an encryption transformer sees compressed bytes.
