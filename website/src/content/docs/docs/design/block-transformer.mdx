...
For encryption, authenticate the ciphertext inside the transformer. SlateDB's CRC32 catches accidental corruption, not malicious modification. The CRC is not a substitute for an AEAD tag or another cryptographic integrity check.
...
SlateDB does not manage encryption keys or transformer versions. If you rotate keys, the transformer needs enough information in each transformed block to choose the right key during `decode`, or every process must keep old keys available until compaction rewrites all files that used them.

Because compaction rewrites SSTs, a rotation plan usually has two phases: deploy readers and compactors that can read both old and new blocks, then switch writers and compactors to emit the new format. Keep the old decode path until garbage collection has removed files written with the old format.
