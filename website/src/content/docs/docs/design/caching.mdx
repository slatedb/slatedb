...
[`ReadOptions::cache_data_blocks`](https://docs.rs/slatedb/latest/slatedb/config/struct.ReadOptions.html#structfield.cache_data_blocks) and [`ScanOptions::cache_data_blocks`](https://docs.rs/slatedb/latest/slatedb/config/struct.ScanOptions.html#structfield.cache_data_blocks) decide whether SlateDB inserts a decoded data block after fetching it on a miss. They do not disable cache lookups. Reads and scans cache SST metadata, such as indexes, filters, and stats, independently of `cache_data_blocks`.

The defaults reflect the usual access patterns. Point reads default to `cache_data_blocks = true` because they are more likely to revisit hot data. Scans default to `cache_data_blocks = false` so a long sequential read does not fill the cache with data blocks that probably will not be reused soon. Scans can still benefit from entries that are already hot. Internal tasks follow the same idea: WAL replay and compaction read SSTs without populating the foreground cache.
