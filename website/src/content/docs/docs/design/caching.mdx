...
The block cache is different. Both [`DbBuilder::with_db_cache`](https://docs.rs/slatedb/latest/slatedb/db/struct.DbBuilder.html#method.with_db_cache) and [`DbReaderBuilder::with_db_cache`](https://docs.rs/slatedb/latest/slatedb/db/struct.DbReaderBuilder.html#method.with_db_cache) take a [`DbCacheAndScope`](https://docs.rs/slatedb/latest/slatedb/db_cache/struct.DbCacheAndScope.html), which pairs your cache object with a `db_cache_id: u64`. This allows you to reuse the same process-local cache implementation across builders if you wish to share a single pool of memory. `db_cache_id` keeps separate `Db` entries from colliding with one another. Pass different ids for logically different databases. Use the same id across a legitimate reopen of the same database to recover its warm (persisted) entries.
