...
```rust
use slatedb::{Db, MetricLevel, Settings};
...
`MetricLevel::Info` is the default. Set `Settings::metric_level` to
`MetricLevel::Debug` to register debug-level metrics; metrics below the
configured level are no-op handles.
