...
- Range scans: Iterating over a key range (e.g., all keys starting with
  `user:123:`) requires no random access, only sequential reads through sorted data.
...
work well because related data shares prefixes and lands in the same blocks.
...
| UUID version | Sort behavior |
|--------------|---------------|
| UUIDv7 | Time-ordered and lexicographically sortable. Prefer it for new applications. |
| UUIDv4 | Random distribution. It does not sort by time. |
...
- Keep keys short. Shorter keys use less memory, compare faster, and leave more room in cache.
- Design for access patterns. Structure keys so common queries become range scans.
- Co-locate related data. Items that are often read together should share key prefixes.
...
- [FoundationDB Data Modeling](https://apple.github.io/foundationdb/data-modeling.html): Tuple encoding, integers, floats, and composite types
