...
`Settings::metric_level` controls which registered metrics are active. The
default is `Info`; set it to `Debug` to enable debug-level metrics when using a
metrics recorder. See [Metric levels](./metrics/#metric-levels) for more
information.
