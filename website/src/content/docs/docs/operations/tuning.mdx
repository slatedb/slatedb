...
* **`min_filter_keys`**: SlateDB only builds bloom filters for SSTs with at least this many keys. Raising it avoids filter overhead on small SSTs.
* **`filter_bits_per_key`**: Controls bloom filter density. Higher values reduce false positives but increase filter size.
