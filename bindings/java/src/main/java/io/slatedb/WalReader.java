 ...
 */
public class WalReader implements AutoCloseable, WalReaderInterface {
  ...
  public WalReader(Pointer pointer) {
   ...
   */
  public WalReader(NoPointer noPointer) {
    this.pointer = null;
    this.cleanable = UniffiLib.CLEANER.register(this, new UniffiCleanAction(pointer));
  }
  ...
  public <R> R callWithPointer(Function<Pointer, R> block) {
    ...
    do {
      ...
      if (c == 0L) {
        throw new IllegalStateException("WalReader object has already been destroyed");
      }
      if (c == Long.MAX_VALUE) {
        throw new IllegalStateException("WalReader call counter would overflow");
      }
  ...
  Pointer uniffiClonePointer() {
    ...
    UniffiHelpers.uniffiRustCallWithError(new SlatedbExceptionErrorHandler(), _status -> {
        UniffiLib.INSTANCE.uniffi_slatedb_ffi_fn_method_walreader_close(
            it, _status);
    });
    ...
    public CompletableFuture<List<WalFile>> list(Long startId, Long endId){
        ...
        callWithPointer(thisPtr -> {
            return UniffiLib.INSTANCE.uniffi_slatedb_ffi_fn_method_walreader_list(
                thisPtr,
                FfiConverterOptionalLong.INSTANCE.lower(startId), FfiConverterOptionalLong.INSTANCE.lower(endId)
            );
        }),
