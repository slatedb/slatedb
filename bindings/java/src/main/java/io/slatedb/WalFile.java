 ...
 * A WAL file handle.
 */
public class WalFile implements AutoCloseable, WalFileInterface {
  ...
  public WalFile(Pointer pointer) {
   ...
   */
  public WalFile(NoPointer noPointer) {
    this.pointer = null;
    this.cleanable = UniffiLib.CLEANER.register(this, new UniffiCleanAction(pointer));
  }
  ...
  public <R> R callWithPointer(Function<Pointer, R> block) {
    ...
    do {
      ...
      if (c == 0L) {
        throw new IllegalStateException("WalFile object has already been destroyed");
      }
      if (c == Long.MAX_VALUE) {
        throw new IllegalStateException("WalFile call counter would overflow");
      }
  ...
  Pointer uniffiClonePointer() {
    ...
    UniffiHelpers.uniffiRustCallWithError(new SlatedbExceptionErrorHandler(), _status -> {
        UniffiLib.INSTANCE.uniffi_slatedb_ffi_fn_method_walfile_close(
            it, _status);
    });
    ...
    public CompletableFuture<WalFileIterator> iterator(){
        ...
        callWithPointer(thisPtr -> {
            return UniffiLib.INSTANCE.uniffi_slatedb_ffi_fn_method_walfile_iterator(
                thisPtr
    ...
    public CompletableFuture<WalFileMetadata> metadata(){
        return UniffiAsyncHelpers.uniffiRustCallAsync(
        callWithPointer(thisPtr -> {
            return UniffiLib.INSTANCE.uniffi_slatedb_ffi_fn_method_walfile_metadata(
                thisPtr
    ...
    public WalFile nextFile()  {
            try {
                return FfiConverterTypeWalFile.INSTANCE.lift(
    callWithPointer(it -> {
        try {
