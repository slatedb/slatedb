...
public class FfiLogCallbackImpl implements AutoCloseable, FfiLogCallback {
  ...
  public FfiLogCallbackImpl(Pointer pointer) {
   ...
   */
  public FfiLogCallbackImpl(NoPointer noPointer) {
    this.pointer = null;
    this.cleanable = UniffiLib.CLEANER.register(this, new UniffiCleanAction(pointer));
  }
  ...
  public <R> R callWithPointer(Function<Pointer, R> block) {
    ...
    do {
      ...
      if (c == 0L) {
        throw new IllegalStateException("FfiLogCallbackImpl object has already been destroyed");
      }
      if (c == Long.MAX_VALUE) {
        throw new IllegalStateException("FfiLogCallbackImpl call counter would overflow");
      }
    ...
    public void run() {
      if (pointer != null) {
        UniffiHelpers.uniffiRustCall(
            status -> {
              UniffiLib.INSTANCE.uniffi_slatedb_ffi_fn_free_ffilogcallback(pointer, status);
              return null;
            });
      }
    }
  ...
  Pointer uniffiClonePointer() {
    return UniffiHelpers.uniffiRustCall(
        status -> {
          ...
          return UniffiLib.INSTANCE.uniffi_slatedb_ffi_fn_clone_ffilogcallback(pointer, status);
        });
  }
      ...
      callWithPointer(
          it -> {
            try {
              ...
              UniffiHelpers.uniffiRustCall(
                  _status -> {
                    UniffiLib.INSTANCE.uniffi_slatedb_ffi_fn_method_ffilogcallback_log(
                        it, FfiConverterTypeFfiLogRecord.INSTANCE.lower(record), _status);
                  });
