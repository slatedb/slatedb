...
public class FfiObjectStore implements AutoCloseable, FfiObjectStoreInterface {
  ...
  public FfiObjectStore(Pointer pointer) {
   ...
   */
  public FfiObjectStore(NoPointer noPointer) {
    this.pointer = null;
    this.cleanable = UniffiLib.CLEANER.register(this, new UniffiCleanAction(pointer));
  }
  ...
  public <R> R callWithPointer(Function<Pointer, R> block) {
    ...
    do {
      ...
      if (c == 0L) {
        throw new IllegalStateException("FfiObjectStore object has already been destroyed");
      }
      if (c == Long.MAX_VALUE) {
        throw new IllegalStateException("FfiObjectStore call counter would overflow");
      }
    ...
    public void run() {
      if (pointer != null) {
        UniffiHelpers.uniffiRustCall(
            status -> {
              UniffiLib.INSTANCE.uniffi_slatedb_ffi_fn_free_ffiobjectstore(pointer, status);
              return null;
            });
      }
    }
  ...
  Pointer uniffiClonePointer() {
    return UniffiHelpers.uniffiRustCall(
        status -> {
          ...
          return UniffiLib.INSTANCE.uniffi_slatedb_ffi_fn_clone_ffiobjectstore(pointer, status);
        });
  }

  public static FfiObjectStore fromEnv(String envFile) throws FfiException {
    try {
      return FfiConverterTypeFfiObjectStore.INSTANCE.lift(
          UniffiHelpers.uniffiRustCallWithError(
              new FfiExceptionErrorHandler(),
              _status -> {
                return UniffiLib.INSTANCE.uniffi_slatedb_ffi_fn_constructor_ffiobjectstore_from_env(
  ...
  public static FfiObjectStore resolve(String url) throws FfiException {
    try {
      return FfiConverterTypeFfiObjectStore.INSTANCE.lift(
          UniffiHelpers.uniffiRustCallWithError(
              new FfiExceptionErrorHandler(),
              _status -> {
                return UniffiLib.INSTANCE.uniffi_slatedb_ffi_fn_constructor_ffiobjectstore_resolve(
