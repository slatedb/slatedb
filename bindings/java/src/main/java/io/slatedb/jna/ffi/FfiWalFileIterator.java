...
public class FfiWalFileIterator implements AutoCloseable, FfiWalFileIteratorInterface {
  ...
  public FfiWalFileIterator(Pointer pointer) {
   ...
   */
  public FfiWalFileIterator(NoPointer noPointer) {
    this.pointer = null;
    this.cleanable = UniffiLib.CLEANER.register(this, new UniffiCleanAction(pointer));
  }
  ...
  public <R> R callWithPointer(Function<Pointer, R> block) {
    ...
    do {
      ...
      if (c == 0L) {
        throw new IllegalStateException("FfiWalFileIterator object has already been destroyed");
      }
      if (c == Long.MAX_VALUE) {
        throw new IllegalStateException("FfiWalFileIterator call counter would overflow");
      }
    ...
    public void run() {
      if (pointer != null) {
        UniffiHelpers.uniffiRustCall(
            status -> {
              UniffiLib.INSTANCE.uniffi_slatedb_ffi_fn_free_ffiwalfileiterator(pointer, status);
              return null;
            });
      }
    }
  ...
  Pointer uniffiClonePointer() {
    return UniffiHelpers.uniffiRustCall(
        status -> {
          ...
          return UniffiLib.INSTANCE.uniffi_slatedb_ffi_fn_clone_ffiwalfileiterator(pointer, status);
        });
  }
  ...
  public CompletableFuture<FfiRowEntry> next() {
    return UniffiAsyncHelpers.uniffiRustCallAsync(
        callWithPointer(
            thisPtr -> {
              return UniffiLib.INSTANCE.uniffi_slatedb_ffi_fn_method_ffiwalfileiterator_next(
                  thisPtr);
            }),
        ...
        // lift function
        (it) -> FfiConverterOptionalTypeFfiRowEntry.INSTANCE.lift(it),
        // Error FFI converter
          ...
          it -> {
            try {
              ...
              UniffiHelpers.uniffiRustCallWithError(
                  new FfiExceptionErrorHandler(),
                  _status -> {
                    UniffiLib.INSTANCE.uniffi_slatedb_ffi_fn_method_ffiwalfileiterator_shutdown(
                        it, _status);
                  });
