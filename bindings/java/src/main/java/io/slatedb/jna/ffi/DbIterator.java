...
public class DbIterator implements AutoCloseable, DbIteratorInterface {
  ...
  public DbIterator(Pointer pointer) {
   ...
   */
  public DbIterator(NoPointer noPointer) {
    this.pointer = null;
    this.cleanable = UniffiLib.CLEANER.register(this, new UniffiCleanAction(pointer));
  }
  ...
  public <R> R callWithPointer(Function<Pointer, R> block) {
    ...
    do {
      ...
      if (c == 0L) {
        throw new IllegalStateException("DbIterator object has already been destroyed");
      }
      if (c == Long.MAX_VALUE) {
        throw new IllegalStateException("DbIterator call counter would overflow");
      }
    ...
    public void run() {
      if (pointer != null) {
        UniffiHelpers.uniffiRustCall(
            status -> {
              UniffiLib.INSTANCE.uniffi_slatedb_uniffi_fn_free_dbiterator(pointer, status);
              return null;
            });
      }
    }
  ...
  Pointer uniffiClonePointer() {
    return UniffiHelpers.uniffiRustCall(
        status -> {
          ...
          return UniffiLib.INSTANCE.uniffi_slatedb_uniffi_fn_clone_dbiterator(pointer, status);
        });
  }
  ...
  public CompletableFuture<KeyValue> next() {
    return UniffiAsyncHelpers.uniffiRustCallAsync(
        callWithPointer(
            thisPtr -> {
              return UniffiLib.INSTANCE.uniffi_slatedb_uniffi_fn_method_dbiterator_next(thisPtr);
            }),
  ...
  public CompletableFuture<Void> seek(byte[] key) {
    return UniffiAsyncHelpers.uniffiRustCallAsync(
        callWithPointer(
            thisPtr -> {
              return UniffiLib.INSTANCE.uniffi_slatedb_uniffi_fn_method_dbiterator_seek(
