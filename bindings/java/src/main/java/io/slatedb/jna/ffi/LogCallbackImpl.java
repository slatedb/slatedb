...
public class LogCallbackImpl implements AutoCloseable, LogCallback {
  ...
  public LogCallbackImpl(Pointer pointer) {
   ...
   */
  public LogCallbackImpl(NoPointer noPointer) {
    this.pointer = null;
    this.cleanable = UniffiLib.CLEANER.register(this, new UniffiCleanAction(pointer));
  }
  ...
  public <R> R callWithPointer(Function<Pointer, R> block) {
    ...
    do {
      ...
      if (c == 0L) {
        throw new IllegalStateException("LogCallbackImpl object has already been destroyed");
      }
      if (c == Long.MAX_VALUE) {
        throw new IllegalStateException("LogCallbackImpl call counter would overflow");
      }
