...
public class FfiWalReader implements AutoCloseable, FfiWalReaderInterface {
  ...
  public FfiWalReader(Pointer pointer) {
   ...
   */
  public FfiWalReader(NoPointer noPointer) {
    this.pointer = null;
    this.cleanable = UniffiLib.CLEANER.register(this, new UniffiCleanAction(pointer));
  }

  public FfiWalReader(String path, FfiObjectStore objectStore) {
    this(
        (Pointer)
            UniffiHelpers.uniffiRustCall(
                _status -> {
                  return UniffiLib.INSTANCE.uniffi_slatedb_ffi_fn_constructor_ffiwalreader_new(
  ...
  public <R> R callWithPointer(Function<Pointer, R> block) {
    ...
    do {
      ...
      if (c == 0L) {
        throw new IllegalStateException("FfiWalReader object has already been destroyed");
      }
      if (c == Long.MAX_VALUE) {
        throw new IllegalStateException("FfiWalReader call counter would overflow");
      }
  ...
  private class UniffiCleanAction implements Runnable {
    ...
    public void run() {
      if (pointer != null) {
        UniffiHelpers.uniffiRustCall(
            status -> {
              UniffiLib.INSTANCE.uniffi_slatedb_ffi_fn_free_ffiwalreader(pointer, status);
              return null;
            });
      }
    }
  ...
  Pointer uniffiClonePointer() {
    return UniffiHelpers.uniffiRustCall(
        status -> {
          ...
          return UniffiLib.INSTANCE.uniffi_slatedb_ffi_fn_clone_ffiwalreader(pointer, status);
        });
  }
          ...
          callWithPointer(
              it -> {
                try {
                  ...
                  return UniffiHelpers.uniffiRustCall(
                      _status -> {
                        return UniffiLib.INSTANCE.uniffi_slatedb_ffi_fn_method_ffiwalreader_get(
  ...
  public CompletableFuture<List<FfiWalFile>> list(Long startId, Long endId) {
    return UniffiAsyncHelpers.uniffiRustCallAsync(
        callWithPointer(
            thisPtr -> {
              return UniffiLib.INSTANCE.uniffi_slatedb_ffi_fn_method_ffiwalreader_list(
                  thisPtr,
        ...
        // lift function
        (it) -> FfiConverterSequenceTypeFfiWalFile.INSTANCE.lift(it),
        // Error FFI converter
          ...
          it -> {
            try {
              ...
              UniffiHelpers.uniffiRustCallWithError(
                  new FfiExceptionErrorHandler(),
                  _status -> {
                    UniffiLib.INSTANCE.uniffi_slatedb_ffi_fn_method_ffiwalreader_shutdown(
                        it, _status);
                  });
