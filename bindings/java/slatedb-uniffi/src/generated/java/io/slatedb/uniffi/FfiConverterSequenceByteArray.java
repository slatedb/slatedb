...
public enum FfiConverterSequenceByteArray implements FfiConverterRustBuffer<List<byte[]>> {
  ...
  public List<byte[]> read(ByteBuffer buf) {
    int len = buf.getInt();
    return IntStream.range(0, len).mapToObj(_i -> FfiConverterByteArray.INSTANCE.read(buf)).toList();
  }

  @Override
  public long allocationSize(List<byte[]> value) {
    long sizeForLength = 4L;
    long sizeForItems = value.stream().mapToLong(inner -> FfiConverterByteArray.INSTANCE.allocationSize(inner)).sum();
