...
The other two motivations — alignment of compaction decisions to segment lifecycle and atomic segment-level retention — still require segment identity somewhere. A sufficiently segment-aware leveled design could preserve segment-aligned file boundaries, but the compactor must deliberately split outputs on those boundaries, the scheduler must reconstruct per-segment state, and the write path must enforce the antichain invariant (see [Validation](#validation)) against the current segment set. To drop an hour atomically, something has to know which files and manifest state belong to that hour. Leveled compaction avoids repeated rewrites but doesn't eliminate segment semantics — it relocates them. The design question is whether segment identity is implicit convention reconstructed from SST key ranges, or explicit manifest data that retention, scheduling, validation, and future lifecycle features consume directly.
