...
mod tests {
    ...
    use super::*;
    use slatedb::{DbBuilder, ErrorKind};

    #[tokio::test(flavor = "current_thread")]
    async fn harness_requires_with_db() {
        let err = Harness::builder(Config::new(7)).run().await.unwrap_err();

        assert!(matches!(err.kind(), ErrorKind::Invalid));
        assert!(
            err.to_string().contains("requires with_db"),
            "unexpected error: {err}"
        );
    }

    #[test]
    fn actor_seed_derivation_is_order_independent() {
        let a = derive_seed(42, "actor", "writer", 0);
        let b = derive_seed(42, "actor", "writer", 0);
        let c = derive_seed(42, "actor", "writer", 1);
        let d = derive_seed(42, "actor", "reader", 0);
    ...
    #[tokio::test(flavor = "current_thread")]
    async fn harness_runs_actor() {
        let runs = Arc::new(AtomicUsize::new(0));

        Harness::builder(Config::new(11))
            .with_db(|ctx| async move {
                let builder = DbBuilder::new(ctx.path().clone(), ctx.main_object_store())
            ...
            .actor_with_state("smoke", 1, Arc::clone(&runs), |ctx, runs| async move {
                assert_eq!(ctx.role(), "smoke");
