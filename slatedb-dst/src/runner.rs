//! Scenario runner framework for deterministic SlateDB simulations.
...
use crate::history::{HistorySnapshot, SqliteHistory};
...
pub trait Scenario {
    ...
    ///
    /// The name is recorded in SQLite history rows and included in mismatch
    ...
    /// point-in-time access to the recorded SQLite history, the shared mock
...
impl ScenarioWriteBatch {
    ...
    /// SlateDB and to the SQLite history.
...
///   recorded SQLite history chosen by the caller;
/// - access to the shared [`MockSystemClock`];
/// - a raw [`Db`] reference for unchecked experiments outside the recorded-history contract.
///
/// Any unchecked operation performed through [`ScenarioContext::db`] is not
/// reflected in the recorded SQLite history automatically. If unchecked
...
impl ScenarioContext {
    ...
    ///
    /// This is an escape hatch for experiments that are outside the checked
    /// mutation API surface. Operations performed directly on the returned
    /// database are not recorded in SQLite history.
    pub fn db(&self) -> &Db {
        &self.shared.db
    }

    /// Returns the shared mock clock used by this simulation run.
    ///
    ...
    /// Returns a point-in-time snapshot of the recorded SQLite history.
    ///
    ...
    pub fn as_of(&self, seq: u64) -> HistorySnapshot {
        HistorySnapshot::new(self.shared.history.clone(), seq)
    }

    /// Waits for the recorded SQLite history to include all writes through `seq`.
    ///
    /// Returns `Ok(false)` if the active run is shutting down before the
    /// recorded history catches up, which allows long-lived reader scenarios to
                ...
                changed = rx.changed() => {
                    if changed.is_err() {
                        return Err(Error::internal(
                            "recorded SQLite history frontier channel closed".to_string(),
                        ));
                    }
                }
            }
    ...
    ///
    /// The write is issued against SlateDB with deterministic simulation
...
/// - the SQLite recorded history used for point-in-time validation and post-run
///   inspection.
///
...
impl ScenarioRunner {
    /// Creates a scenario runner with an optional on-disk SQLite history database.
    ///
    /// When `sqlite_path` is `None`, the history lives in memory. Supplying a
    ...
    pub fn new(
        ...
        let history = Arc::new(Mutex::new(SqliteHistory::new(sqlite_path)));
